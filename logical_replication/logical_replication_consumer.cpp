...
void logical_replication_consumer::send_standby_status_update(PGconn *conn,
    ...
    try {
        ...
        current_logger->log_to_console(log_level::WARN, fmt::format("Could not get current time for feedback: {}", e.what()));
         client_time_us = 0;
    }
...
PGconn *logical_replication_consumer::start_replication() {
    ...
    current_logger->log_to_console(log_level::INFO,
...
bool logical_replication_consumer::consume()
{
    ...
    try
    {
        ...
        while (true) {
            ...
            if (bytes_read == -2) {
                current_logger->log_to_console(log_level::ERR, "End of replication stream.");
                break;
            }
            ...
            if (message_type == 'k') { // Primary keep-alive message
                if (len < 1 + 8 + 8 + 1) {
                    current_logger->log_to_console(log_level::ERR, fmt::format("Received truncated KeepAlive message."));
                    continue;
                }
                ...
                if (reply_required) {
                    current_logger->log_to_console(log_level::DEBUGER, fmt::format("Sending immediate feedback. Processed LSN: {}", lsn_to_string(last_processed_lsn)));
                ...
                if (len < 1 + 8 + 8 + 8) {
                    current_logger->log_to_console(log_level::ERR, "Received truncated XLogData message.");
                    continue;
                }
            ...
            if (now - last_feedback_time >= feedback_interval) {
                current_logger->log_to_console(log_level::DEBUGER, fmt::format("Sending periodic feedback. Processed LSN: {}", lsn_to_string(last_processed_lsn)));
                send_standby_status_update(conn, last_received_lsn, last_processed_lsn, last_processed_lsn, false);
        ...
        current_logger->log_to_console(log_level::INFO, "Stopping replication client.");
        if (copy_buf) {
            PQfreemem(copy_buf);
        }
    ...
    {
        current_logger->log_to_console(log_level::ERR, fmt::format("Exception thrown in consume", e.what()));
        return false;
    }
    catch (const std::exception & e)
    {

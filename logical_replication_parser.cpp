                                               ...
                                               bool old_value = false)
{
    ...
    {
        current_logger->log(log_level::DEBUG, fmt::format("Identifier data: {}", identifier_data));
        switch (identifier_data)
        {
            ...
            {
                current_logger->log(log_level::WARNING,
            ...
            default:
            {
                current_logger->log(log_level::WARNING, fmt::format("Unexpected identifier: {}", identifier_data));
                break;
            }
        }
    };
    ...
    for (int16_t column_idx = 0; column_idx < num_columns; ++column_idx)
    {
        ...
        {
            current_logger->log(log_level::ERROR,
                        fmt::format(
                                               ...
                                               std::unordered_map<int32_t, std::string>& old_value)
{
    ...
    current_logger->log(log_level::DEBUG, fmt::format("Message type: {}, lsn string: {}", type, *current_lsn));

    current_logger->log(log_level::DEBUG, fmt::format("Type operation: {}", type));
    ...
    switch (type)
    {
        ...
        case 'I': // Insert
        {
            ...
            current_logger->log(log_level::DEBUG, fmt::format("Table name for insert: ", table_name));

            if (table_name.empty())
            {
                current_logger->log(log_level::WARNING, fmt::format("No table mapping for table id: {}.", table_id));
                return;
            }
        ...
        case 'U': // Update
        {
            table_id = parse_int32(replication_message, pos, size);
            const auto & table_name = id_to_table_name[table_id];
            current_logger->log(log_level::DEBUG, fmt::format("Table name for update: ", table_name));
        ...
        case 'D': // Delete
        {
            table_id = parse_int32(replication_message, pos, size);
            const auto & table_name = id_to_table_name[table_id];
            current_logger->log(log_level::DEBUG, fmt::format("Table name for delete: ", table_name));
        ...
        {
            table_id = parse_int32(replication_message, pos, size);
            current_logger->log(log_level::DEBUG, fmt::format("Table id: {}", table_id));
            ...
            current_logger->log(log_level::DEBUG, fmt::format("Table name: {}", table_name));
            ...
            {
                current_logger->log(log_level::WARNING, fmt::format("Invalid identity: {}", table_identity));
            ...
            for (uint16_t i = 0; i < num_columns; ++i)
            {
                ...
                current_logger->log(log_level::DEBUG, fmt::format("Column name: {}", column_name));
